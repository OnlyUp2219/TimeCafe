...
describe("GET /venue/tariffs/page", () => {
    it("returns endpoint result when authorized", async () => {
        const {client, headers} = await createAuthorizedContext();
        const res = await client.get("/venue/tariffs/page", {headers, params: {pageNumber: 1, pageSize: 10}});
        expect(res.status).not.toBe(401);
    });

    it("returns 401 when unauthorized", async () => {
        const {client} = createTestClient();
        const res = await client.get("/venue/tariffs/page", {params: {pageNumber: 1, pageSize: 10}});
