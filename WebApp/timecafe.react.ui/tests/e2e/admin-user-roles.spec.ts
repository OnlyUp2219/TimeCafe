...
test.describe("Admin User Detail and Roles E2E Flow", () => {
    test("Admin navigates to user details and role configuration", async ({browser}) => {
        ...
        const openButton = adminPage.getByRole("button", {name: "Открыть"}).first();
        await expect(openButton).toBeVisible({timeout: 15000});
        await openButton.click();

        await expect(adminPage.getByText("Назад к списку").first()).toBeVisible({timeout: 15000});
        await expect(adminPage.getByText("Программа лояльности").first()).toBeVisible();

        await adminPage.getByRole("button", {name: "Управление ролями"}).first().click();

        await expect(adminPage.getByText("Роли пользователя").first()).toBeVisible({timeout: 15000});
