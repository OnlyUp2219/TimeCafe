...
public class ActivatePromotionCommandTests : BaseCqrsHandlerTest
{
    ...
    {
        var command = new ActivatePromotionCommand(TestData.NonExistingIds.NonExistingPromotionId);
    ...
    public async Task Handler_Should_ReturnFailed_WhenRepositoryReturnsFalse()
    {
        ...
        PromotionRepositoryMock.Setup(r => r.GetByIdAsync(promotionId, It.IsAny<CancellationToken>())).ReturnsAsync(promotion);
        PromotionRepositoryMock.Setup(r => r.ActivateAsync(promotionId, It.IsAny<CancellationToken>())).ReturnsAsync(false);
    ...
    {
        var command = new ActivatePromotionCommand(Guid.Parse(promotionIdStr));
