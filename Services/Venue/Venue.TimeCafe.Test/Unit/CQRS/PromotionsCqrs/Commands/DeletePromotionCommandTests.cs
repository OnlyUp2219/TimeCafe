...
public class DeletePromotionCommandTests : BaseCqrsHandlerTest
{
    ...
    {
        var command = new DeletePromotionCommand(1);
        var promotion = new Promotion { PromotionId = 1, Name = "Test", Description = "Desc", ValidFrom = DateTime.UtcNow, ValidTo = DateTime.UtcNow.AddDays(30) };

        PromotionRepositoryMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(promotion);
        PromotionRepositoryMock.Setup(r => r.DeleteAsync(1)).ReturnsAsync(true);
    ...
    {
        var command = new DeletePromotionCommand(999);
    ...
    public async Task Handler_Should_ReturnFailed_WhenRepositoryReturnsFalse()
    {
        ...
        PromotionRepositoryMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(promotion);
        PromotionRepositoryMock.Setup(r => r.DeleteAsync(1)).ReturnsAsync(false);
